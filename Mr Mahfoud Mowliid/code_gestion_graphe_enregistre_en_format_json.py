...
from tkinter import messagebox as boite_message
...
def nouveau_fichier():
    ...
    nouvelle_fenetre.title("Nouveau Fichier")
...
def ouvrir_fichier(canvas):
    fichier = dialogue_fichier.askopenfilename(title="Ouvrir un fichier", defaultextension=".json",
...
# Fonction pour enregistrer (met à jour si déjà sauvegardé, sinon demande où sauvegarder)
def enregistrer_fichier(canvas, fenetre_actuelle):
    # Si la fenêtre actuelle n'a pas de chemin de fichier, demandez où enregistrer
    if fenetre_actuelle.chemin_fichier is None:
        fenetre_actuelle.chemin_fichier = dialogue_fichier.asksaveasfilename(
            title="Enregistrer le graphe", defaultextension=".json",
            filetypes=[("JSON files", "*.json"), ("All files", "*.*")]
        )
    ...
    if fenetre_actuelle.chemin_fichier:
        ...
        with open(fenetre_actuelle.chemin_fichier, 'w') as f:
            json.dump(data, f)
        
        boite_message.showinfo("Enregistrement réussi", f"Graphe enregistré sous : {fenetre_actuelle.chemin_fichier}")

# Fonction pour "Enregistrer sous" (toujours demande de fichier)
def enregistrer_sous(canvas, fenetre_actuelle):
    # Toujours demander où sauvegarder
    nouveau_fichier = dialogue_fichier.asksaveasfilename(
        title="Enregistrer sous", defaultextension=".json",
        filetypes=[("JSON files", "*.json"), ("All files", "*.*")]
    )
    
    if nouveau_fichier:  # Si un fichier est sélectionné
        fenetre_actuelle.chemin_fichier = nouveau_fichier  # Mettre à jour le chemin de fichier
...
def creer_fenetre_principale(fenetre): 
    ...
    menu_fichier.add_command(label="Ouvrir", image=icon_ouvrir, compound=LEFT, command=lambda: ouvrir_fichier(canvas))
    menu_fichier.add_command(label="Enregistrer", image=icon_enregistrer, compound=LEFT,
                         command=lambda: enregistrer_fichier(canvas, fenetre))
    menu_fichier.add_command(label="Enregistrer sous", image=icon_enregistrer_sous, compound=LEFT, 
                         command=lambda: enregistrer_sous(canvas, fenetre))

    menu_fichier.add_separator()  # Séparateur dans le menu
    menu_fichier.add_command(label="Fermer", image=icon_fermer, compound=LEFT, command=fermer_fenetre)  # Option Fermer
    fenetre.protocol("WM_DELETE_WINDOW", fermer_fenetre)  # Appeler la fonction de fermeture

    sous_menu_sommet = Menu(menu_creation, tearoff=0)
    sous_menu_sommet.add_command(label="Ajouter Un Sommet", image=icon_sommet, compound=LEFT , command=lambda: ajouter_sommet(canvas))
    sous_menu_sommet.add_command(label="Retirer Un Sommet", image=icon_sommet, compound=LEFT , command=lambda: retirer_sommet(canvas)) 
    ...
    sous_menu_arret.add_command(label="Ajouter Une Arête", image=icon_arret, compound=LEFT , command=lambda: ajouter_arrete(canvas))
    sous_menu_arret.add_command(label="Retirer Une Arête", image=icon_arret, compound=LEFT , command=lambda: retirer_arrete(canvas))
