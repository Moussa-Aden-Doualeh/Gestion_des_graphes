...
def incrementer_nom_sommet():
    ...
    if not liste_sommets:
        return zone_dialogue.askstring("Ajouter un sommet", "Entrez la première lettre ou chiffre du sommet : ")
...
def ajouter_sommet(canvas):
    ...
    nom_sommet_courant = incrementer_nom_sommet()
    ...
    def lors_du_clic(event):
        ...
        texte = canvas.create_text(x, y, text=nom_sommet_courant, fill="white", font=("Arial", 12, "bold"))
            ...
            if abs(sx - x) < 25 and abs(sy - y) < 25:
                ...
                if len(selection_sommets) == 2:
                    ...
                    if sommet1 == sommet2:
                        ...
                        if any((sommet1 == s1 and sommet2 == s2) or (sommet1 == s2 and sommet2 == s1) for s1, s2, _, _, _ in arcs):
                            boite_message.showerror("Erreur", "Une arête entre ces deux sommets existe déjà.")
                        else:
                            rayon = 20  # Le même rayon que pour les sommets
                            ...
                            # Ici, l'étiquette est définie en fonction du nombre d'arêtes existantes
                            etiquette = f"M{len(arcs) + 1}"  # Incrémenter l'étiquette
                            cx = (x1_ext + x2_ext) / 2
                            cy = (y1_ext + y2_ext) / 2
                            texte = canvas.create_text(cx + 10, cy - 10, text=etiquette, font=("Arial", 10, "bold"))

                            # Ajout de l'arête à la liste avec tous les détails
                            arcs.append({
                                'sommet1': sommet1,
                                'sommet2': sommet2,
                                'ligne': ligne,
...
# Fonction pour retirer une arête
